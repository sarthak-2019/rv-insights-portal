...
import { companies, callLogs } from "@/data/mockData";
...
export default function Companies() {
  ...
  const companiesWithStats = useMemo(() => {
    return companies.map((company) => {
      const companyLogs = callLogs.filter((log) => log.companyId === company.id);
      return {
        ...company,
        totalCalls: companyLogs.length,
        completedCalls: companyLogs.filter((l) => l.status === "completed").length,
        issues: companyLogs.filter((l) => l.status === "issue").length,
      };
    });
  }, []);
  ...
  }, [companiesWithStats, typeFilter, searchQuery]);
